...
import { CalendarIcon, CloudRain, Snowflake, ThermometerSun, Wind } from "lucide-react"
...
export const DatesSection = ({ lat, lon, radius }: { lat: number | undefined | null, lon: number | undefined | null, radius: number }) => {
    ...
    const mutation = useMutation({
        mutationFn: async () => await weatherService.getWeather({ lat, lon, radius, start_date: formatDate(date), end_date: formatDate(date) }),
        onSuccess: async () => {
    ...
    return (
        <Card className='w-auto absolute bottom-4 left-4 right-4 sm:bottom-8 sm:left-8 sm:right-8 rounded-lg'>
            {/* <CardHeader className="w-fit"> */}
            {/* <CardTitle className='flex items-center gap-2 text-sm'><CalendarIcon className="w-3 h-3" />Date:</CardTitle> */}
            <CardContent className='flex flex-col gap-4 text-sm'>
                ...
                <div className="text-muted-foreground  px-1 text-sm flex items-center justify-between gap-4 flex-wrap ">
                    ...
                    <Button className="ml-auto" onClick={() => mutation.mutate()}>Check Weather!</Button>
                </div>
            </CardContent>
